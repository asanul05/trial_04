...
<script>
    ...
    async function fetchNews() {
        ...
        let url = `../Website/api/announcements/list.php?page=${currentPage}&limit=${currentLimit}&search=${encodeURIComponent(currentSearch)}`;
        ...
        try {
            ...
            if (!response.ok) { // Check for HTTP errors (e.g., 404, 500)
                ...
                showErrorModal('HTTP Error fetching news: ' + response.status + ' - ' + errorText.substring(0, 100) + '...', false, false);
            ...
            const newsContainer = document.getElementById('news-container');
            newsContainer.innerHTML = '<p class="col-span-full text-center text-red-500">Failed to load news.</p>';
            showErrorModal('Network or parsing error fetching news: ' + error.message, false, false);
        } finally {
